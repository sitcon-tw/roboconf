	...
	if assignee is not None:					# empty string => unassign
		if len(assignee) > 0:
			try:
				...
				notify(issue, user, '* %s 已將此議題指派給 %s *' % (request.user.username, u.username))
			...
			notify(issue, user, '* %s 已撤銷此議題指派的人員*' % (request.user.username))
...
def toggle_state(issue, request):
	...
	notify(issue, user, ('* %s 已將此議題結案 *' if issue.is_open else '* %s 已對此議題提出復議 *') % (request.user.username))
